...
include_once '../../backend/models/avaliacao.php';
...
try {
    ...
    if ($stmt = mysqli_prepare($connection, $sql)) {
        if (mysqli_stmt_execute($stmt)) {
            mysqli_stmt_bind_result($stmt, $id, $idRoteiroCompras, $idGrupoVisitas, $avaliacaoGuia, $avaliacaoViagem, $comentario);

            $avaliacoes = [];
            while (mysqli_stmt_fetch($stmt)) {
                $avaliacoes[] = new Avaliacao($id, $idRoteiroCompras, $idGrupoVisitas, $avaliacaoGuia, $avaliacaoViagem, $comentario);
            }
